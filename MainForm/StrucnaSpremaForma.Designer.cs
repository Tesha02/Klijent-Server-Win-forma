namespace View
{
	partial class StrucnaSpremaForma
	{
		/// <summary>
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary>
		/// Clean up any resources being used.
		/// </summary>
		/// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && (components != null))
			{
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region Windows Form Designer generated code

		/// <summary>
		/// Required method for Designer support - do not modify
		/// the contents of this method with the code editor.
		/// </summary>
		private void InitializeComponent()
		{
			label1 = new Label();
			label2 = new Label();
			tbNaziv = new TextBox();
			tbOpis = new TextBox();
			btnDodaj = new Button();
			btnIzmeni = new Button();
			SuspendLayout();
			// 
			// label1
			// 
			label1.AutoSize = true;
			label1.Location = new Point(99, 69);
			label1.Name = "label1";
			label1.Size = new Size(53, 20);
			label1.TabIndex = 0;
			label1.Text = "Naziv: ";
			// 
			// label2
			// 
			label2.AutoSize = true;
			label2.Location = new Point(99, 129);
			label2.Name = "label2";
			label2.Size = new Size(46, 20);
			label2.TabIndex = 1;
			label2.Text = "Opis: ";
			// 
			// tbNaziv
			// 
			tbNaziv.Location = new Point(155, 66);
			tbNaziv.Name = "tbNaziv";
			tbNaziv.Size = new Size(125, 27);
			tbNaziv.TabIndex = 2;
			// 
			// tbOpis
			// 
			tbOpis.Location = new Point(155, 126);
			tbOpis.Name = "tbOpis";
			tbOpis.Size = new Size(125, 27);
			tbOpis.TabIndex = 3;
			// 
			// btnDodaj
			// 
			btnDodaj.Location = new Point(172, 216);
			btnDodaj.Name = "btnDodaj";
			btnDodaj.Size = new Size(94, 29);
			btnDodaj.TabIndex = 4;
			btnDodaj.Text = "Dodaj";
			btnDodaj.UseVisualStyleBackColor = true;
			btnDodaj.Click += btnDodaj_Click;
			// 
			// btnIzmeni
			// 
			btnIzmeni.Location = new Point(172, 216);
			btnIzmeni.Name = "btnIzmeni";
			btnIzmeni.Size = new Size(94, 29);
			btnIzmeni.TabIndex = 5;
			btnIzmeni.Text = "Izmeni";
			btnIzmeni.UseVisualStyleBackColor = true;
			btnIzmeni.Click += btnIzmeni_Click;
			// 
			// StrucnaSpremaForma
			// 
			AutoScaleDimensions = new SizeF(8F, 20F);
			AutoScaleMode = AutoScaleMode.Font;
			ClientSize = new Size(424, 364);
			Controls.Add(btnIzmeni);
			Controls.Add(btnDodaj);
			Controls.Add(tbOpis);
			Controls.Add(tbNaziv);
			Controls.Add(label2);
			Controls.Add(label1);
			Name = "StrucnaSpremaForma";
			Text = "StrucnaSpremaForma";
			ResumeLayout(false);
			PerformLayout();
		}

		#endregion

		private Label label1;
		private Label label2;
		private TextBox tbNaziv;
		private TextBox tbOpis;
		private Button btnDodaj;
		private Button btnIzmeni;
	}
}